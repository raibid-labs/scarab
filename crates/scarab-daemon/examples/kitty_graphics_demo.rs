...
fn demo_simple_png() {
    ...
    if let Some(cmd) = parse_kitty_graphics(sequence.as_bytes()) {
        ...
        println!(
            "  Display: {}x{} cells",
            cmd.display_columns.unwrap_or(0),
            cmd.display_rows.unwrap_or(0)
        );
...
fn demo_chunked_transfer() {
    ...
    if let Some(cmd) = parse_kitty_graphics(chunk1.as_bytes()) {
        ...
        let result = state.add_chunk(cmd.image_id.unwrap(), cmd.payload.clone(), !cmd.more_chunks);
    ...
    if let Some(cmd) = parse_kitty_graphics(chunk2.as_bytes()) {
        ...
        assert!(
            result.is_none(),
            "Second chunk should not complete transfer"
        );
    }
...
fn demo_display_existing() {
    ...
    if let Some(cmd) = parse_kitty_graphics(sequence.as_bytes()) {
        ...
        println!(
            "  Grid position: ({}, {})",
            cmd.grid_x.unwrap_or(0),
            cmd.grid_y.unwrap_or(0)
        );
        println!(
            "  Display size: {}x{} cells",
            cmd.display_columns.unwrap_or(0),
            cmd.display_rows.unwrap_or(0)
        );
...
fn demo_rgb_format() {
    ...
    if let Some(cmd) = parse_kitty_graphics(sequence.as_bytes()) {
        ...
        println!(
            "  Source dimensions: {}x{} pixels",
            cmd.source_width.unwrap_or(0),
            cmd.source_height.unwrap_or(0)
        );
        println!(
            "  Display size: {}x{} cells",
            cmd.display_columns.unwrap_or(0),
        ...
        println!(
            "  Payload size: {} bytes (should be 12 for 2x2 RGB)",
            cmd.payload.len()
        );
