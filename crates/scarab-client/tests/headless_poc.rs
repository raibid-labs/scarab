...
fn test_query_basic_components() {
    ...
    let node = world
        .get::<Node>(entity)
        .expect("Node component should exist");
    let transform = world
...
fn test_mock_assets() {
    ...
    assert!(
        retrieved.is_some(),
        "Asset should be stored in Assets<Image>"
    );
...
fn test_scarab_ui_component() {
    ...
    let mut palette_query = app
        .world_mut()
        .query_filtered::<Entity, With<CommandPaletteMarker>>();
    ...
    let children = app
        ...
        .expect("Palette should have children");
    assert_eq!(
        children.len(),
        4,
        "Should have 1 search input + 3 command items"
    );
